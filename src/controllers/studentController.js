...
async function list(req, res, next) {
  try { return success(res, await service.list(req.query.classId)); } catch (err) { return next(err); }
}
...
async function update(req, res, next) {
  try {
    return success(res, await service.update(Number(req.params.id), req.body), "Data siswa diperbarui");
  } catch (err) { return next(err); }
}
